...
float db_min_dist_old(DB *db, float lat, float lon) {
...
float db_min_dist(DB *db, float lat, float lon) {
    ...
    __m256 base_lon = _mm256_set1_ps(lon);
    __m256 dists = _mm256_set1_ps(1.0/0.0);
    // print_vec("base lat", base_lat);

    for (int i = 0; i < size; i+=8) {
        __m256 lats = _mm256_load_ps(&latitudes[i]);
        __m256 lons = _mm256_load_ps(&longitudes[i]);
        // print_vec("latitudes", lats);

        __m256 dxs = _mm256_sub_ps(lats, base_lat);
        __m256 dys = _mm256_sub_ps(lons, base_lon);
        // print_vec("dxs", dxs);

        dxs = _mm256_mul_ps(dxs,dxs);
        dys = _mm256_mul_ps(dys,dys);

        __m256 dist = _mm256_add_ps(dxs,dys);
        dists = _mm256_min_ps(dists,dist);
    }
    ...
    float min = min_float_in_vector(dists);
    return sqrtf(min);
}
...
void db_process_single_request(DB * db, Request * req, bool atomic) {
    ...
    if(!atomic) { 
        ...
        atomic_fetch_add(&(db->access_counts[id]), 1);
        //atomic_fetch_add(&(db->global_access_count), 1);
    }
}
...
void * thread_process_requests_base(void * thread_params) {
    ...
    for(int i=params.thread_id; i<params.num_requests; i += params.num_threads) {
        db_process_single_request(params.db, &params.requests[i], true);
    }
...
void * thread_process_requests_split(void * thread_params) {
    ...
    /*
    for (int n_th = 0; n_th<4; n_th++) {
        params.num_threads = n_th;
