...
DB* db_init(Person *persons_array, int num_elements) {
    ...
    unsigned long array_start = (unsigned long) malloc(num_elements * sizeof(Person)+64);
    // 2.K : Décaler le début du tableau pour forcer l'alignement
    array_start = array_start + (64-array_start % 64);
    ...
    printf("%lu %lu\n", address_first % 64, address_second % 64);
