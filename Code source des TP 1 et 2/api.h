...
typedef struct Person {
    char *first_name; // pointer to a string representing the first name, 8 bytes
    char *last_name;  // pointer to a string representing the first name, 8 bytes
    char male;        // 1 for male, 0 for female, 1 byte
    char wizard;      // 1 for wizard, 0 for muggle, 1 byte
    uint32_t age;     // 32 bits unsigned int representing the age, 4 bytes
    float latitude;   // latitude of his last know residence, 4 bytes
    float longitude;  // longitude of his last know residence, 4 bytes
} Person;
